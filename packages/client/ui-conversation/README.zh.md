...
会话页头会在标题旁渲染会话作用域的 `'conversation.session.header.actions'` 列表，并在最右侧渲染独立的 `'conversation.session.header.utilities'` 列表。会话上下文和谱系控件保留在 `actions` 中；可选的会话工具不会改变它们的顺序或位置。内置 Review 工具会打开详情列，并支持 Cmd/Ctrl+Alt+B；其中的变更、仅产出文件和任务区分别来自已加载的对话快照与当前 todo 投影，选中工具调用后还会在下方附加其输入／输出详情。已经出现在变更区的路径不会作为产出文件再次显示。宽度不超过 720px 时，shell 会让出详情轨，因此该工具不显示，其快捷键也不生效。编辑器链的 currency 包含当前对话 `session`；ui-subagent 会选取 one-shot 或 parent 不可用的已寻址会话，并按原因显示只读文案，而普通 InputBar 会让所有已寻址 child 仅保留 Send，因为继续执行服务不公开逐 Activation 取消操作，`session.cancel` 也会绕过其所有权。
