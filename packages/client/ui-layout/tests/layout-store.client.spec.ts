...
describe('createLayoutStore', () => {
  it('initializes the sidebar at its default width, details closed, wide viewport assumed', () => {
    ...
    expect(store.getSnapshot()).toEqual({ sidebar: SIDEBAR_DEFAULT, details: 0, narrow: false, narrowExpanded: false, page: 'home' })
  })
  ...
  it('narrow toggleSidebar flips only the re-expand override; the width preference survives', () => {
    ...
    actions.toggleSidebar()
    expect(store.getSnapshot()).toEqual({ sidebar: 400, details: 0, narrow: true, narrowExpanded: true, page: 'home' })
    actions.toggleSidebar()
  ...
  it('showPage writes a new page id and ignores a repeat', () => {
    const { store, actions } = createLayoutStore().create()
    actions.showPage('jobs')
    expect(store.getSnapshot().page).toBe('jobs')
    actions.showPage('jobs')
    expect(store.getSnapshot().page).toBe('jobs')
    actions.showPage('home')
    expect(store.getSnapshot().page).toBe('home')
  })
})
