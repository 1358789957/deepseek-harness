...
- **Concession-chain auto-close derives a zero width without touching the preferred width** — the panel restores itself when the window widens. The details owner share (`open`, `available`) and `data-details-available` are the rendered truth; the stored details width is only the preference.
