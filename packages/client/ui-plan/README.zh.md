...
plan mode 经 `/plan` 命令路径进入：加号菜单的 Plan 行是始终可见的开／关切换（`/plan` 与 `/plan off`），用户仍可键入 `/plan`。当 host 计算的 `plan` 投影有效目标为 plan mode 时（`pending ? !active : active`——折叠的 host 值而非客户端乐观态，帧到达即自动纠正），`conversation.input.plan` 座位渲染 warn 色的 "Plan ×" 状态按钮，该按钮经 `command.execute` 执行 `/plan off`；否则该座位保持为空——未组合 plan-mode 的 host（或尚无会话的 Draft）不显示 chip。加号行在这些状态下仍会渲染，并在没有会话或 composer 锁定时禁用切换。plan mode 为有效目标期间，composer 文本框的 placeholder 切换为 plan 任务提示——"describe your task to generate plan"（中文「描述你的任务以生成计划」），经 ui-conversation 的 `conversation` locale 命名空间（`placeholder.plan` / `hint.plan` 键）本地化，并与已认领 `/plan` 命令的提示逐字共用同一份文案（由 composer 从同一投影渲染；owner 提供的 placeholder 优先）。
...
- **plan mode 关闭时状态 chip 仍隐藏**——加号菜单行是未激活态的开关；工具行 chip 仍只负责退出。
