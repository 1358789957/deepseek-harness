...
`SidebarRootComponentProps` 组合布局 owner share、全局 `useSessions` 和 `useWorkspaces` 钩子、已声明的 `sidebar.nav`、`sidebar.workspaces` 与 `sidebar.settings` 子 slot，以及注入的 `startSession` 与侧边栏切换回调。`sidebar.nav` 是「新会话」与工作区浏览器之间、仅宽栏渲染的列表；其 owner share 为 `{ wide, page }`。这里没有插件 store。
