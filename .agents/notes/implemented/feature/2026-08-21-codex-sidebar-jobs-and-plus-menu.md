...
- `packages/client/ui-conversation/tests/input-bar.client.spec.tsx` and `plus-menu-placement.client.spec.ts` pin plus → local menu, right / left placement, `data-plus-open`, no `toggleCommandMenu`, Escape / outside / session change close, and disabled-only-when-removed.
