...
The Skill page lists `skill.list` (host cwd when there is no session) with `source`, and refetches on forwarded `skills/change`. Left nav splits 内置 (`bundled`, `project-agents`) from 导入. Each row has a checkbox; new names default on; `dsh.skill-enabled` stores the off set; unchecked rows stay listed and leave the slash catalog. A solid 导入 SKILL.md button (`label-primary` fill) picks a file into `dsh.imported-skills`; a later host row of the same name wins. Empty copy is per group.
