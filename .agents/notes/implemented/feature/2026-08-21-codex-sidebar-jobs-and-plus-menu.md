...
The plus control still opened the slash Command menu, so Plan, the four agent modes, and Skill had no always-visible composer home. Skill must open a full page, not Settings → Plugins. The API key is typed on Settings → Models: write-only, never echoed.
...
Plus rows, in order: Plan toggle (开/关, `/plan` / `/plan off`); the four built-in modes 标准 / PTC / 极简 / 创造 (`standard` / `code` / `minimal` / `cordis`, else the first four roster options); Skill with a right chevron that closes the menu and `showPage('skills')`.

Skill must not live as local state inside a plus item: `onClose` unmounts plus entries. Skill is `shell.page` id `skills`.

The Skill page lists `skill.list` (host launch cwd through the default preset's standing layer when there is no session) with `source`, and refetches on forwarded `skills/change`. Left nav splits 内置 (`bundled`, `project-agents`) from 导入. A search field filters the open group; the solid 导入 SKILL.md button (`label-primary` fill) sits to its right as the add action, never inside the list. Each row has a checkbox; new names default on; `dsh.skill-enabled` stores the off set; unchecked rows stay listed and leave the slash catalog. A pick prepends `dsh.imported-skills` (newest first); a later host row of the same name wins. Empty copy for 导入 is 「还没有导入的技能。装上的会按导入顺序排在这里。」

API key writes go through `credentials.set` / `unset` on the Models page `ProviderEditor` (and the DeepSeek onboarding step). `credentials.describe` is value-free. After save the password field clears and the row shows a configured badge. The page never logs the key, never reads a machine environment key, and never writes plaintext into the composer menu.
...
**Keep plus as the slash Command launcher.** Rejected. The plus menu was empty of Plan, modes, and Skill; `/` already opens commands.

**Open Skill as Settings → Plugins.** Rejected. The right chevron opens a full `shell.page`, not a settings section.

**Store the API key in `localStorage` or echo it after save.** Rejected. The official store is write-only `credentials.set` on Settings → Models; the editor never holds the secret after save.

**Put API 密钥 in the plus menu.** Rejected. Settings → Models already owns the write-only `ProviderEditor`; the plus list is Plan, modes, and Skill.
...
**Keep Skill as plus-item local state.** Rejected. `onClose` unmounts the menu; the Skill page must outlive it.
...
- `dsh web` on this fork loads `ui-jobs`, `ui-skill`, `ui-plan`, `ui-agent-preset`, and `ui-settings-models` from the web-app roster; `preview/codex-shell.html` is not the served GUI.
...
- `packages/client/ui-settings-models/tests/apply.client.spec.ts` pins that Models does not occupy `conversation.input.plus`; `components.client.spec.tsx` pins write-only `ProviderEditor` save without echoing the secret.
