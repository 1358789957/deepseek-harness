...
Skill 页列出 `skill.list`（没有会话时列出宿主 cwd）并带 `source`，在转发的 `skills/change` 上重新拉取。左侧导航把内置（`bundled`、`project-agents`）和导入分开。每行有复选框；新名称默认勾选；`dsh.skill-enabled` 只存关掉的集合；未勾选的行仍列出，但不进 slash 目录。实心「导入 SKILL.md」按钮（`label-primary` 填充）把选中的文件写入 `dsh.imported-skills`；之后同名的宿主行优先。空态按分组。
